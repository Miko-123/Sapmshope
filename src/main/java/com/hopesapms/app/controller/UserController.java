...
public class UserController {
        ...
        @GetMapping("all-students/{roleName}")
        @PreAuthorize("hasAuthority('INSTRUCTOR')")
        @Operation(summary = "Get all active students for logged in instructor", description = "Retrieves a paginated list of active users (Instructore only)")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "List of users", content = @Content(schema = @Schema(implementation = Page.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden: Requires Instructore authority")
        })
        public ResponseEntity<Page<UserResponseDTO>> getAllStudents(@PathVariable String roleName, @ParameterObject @PageableDefault(size = 10, page = 0) Pageable pageable) {
                return ResponseEntity.ok(userService.getAllActiveStudents(roleName, pageable));
        }
