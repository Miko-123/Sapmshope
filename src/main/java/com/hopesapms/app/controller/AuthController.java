...
public class AuthController {
    ...
    public ResponseEntity<MessageResponseDTO> verifyCode(@RequestParam("email") String eamil, @RequestParam("code") String code) {
        boolean success = otpService.validateOtp(eamil, code);
        if (success) {
            verificationService.cacheVerifiedEmail(eamil);
            return ResponseEntity.ok(new MessageResponseDTO("Code verified successfully. Please complete your profile."));
        } else {
