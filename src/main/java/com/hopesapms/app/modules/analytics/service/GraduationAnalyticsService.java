...
import com.hopesapms.app.common.util.GpaCalculationUtil;
...
public class GraduationAnalyticsService {
    ...
    private List<StudentAnalysis> analyzeAllStudents() {
        ...
        List<Student> students = studentRepository.findAllByIsDeletedFalse(Pageable.unpaged())
                .getContent()
                .stream()
                .filter(s -> s.getYearLevel() >= 4 && "ACTIVE".equalsIgnoreCase(s.getStatus()))
